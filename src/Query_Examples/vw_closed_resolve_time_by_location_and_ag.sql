 SELECT
    ...
    incident.state,
    round(avg(incident.business_duration::numeric / 86400::numeric)) AS avg_resolve_time_indays
   FROM incident
     LEFT JOIN cmn_location ON incident.location::text = cmn_location.name::text
  WHERE incident.active = false AND incident.business_duration IS NOT NULL
